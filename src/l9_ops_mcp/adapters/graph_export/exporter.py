...
class ArtifactNode:
    ...
    def validate(self) -> None:
        ...
        if self.callable and self.mcp_primitive in {"none", ""}:
...
def node_from_index_item(
    item: dict[str, Any], repo_root: Path, namespace: str = "core"
) -> ArtifactNode:
    source_path = str(item.get("source_path") or item.get("path") or item.get("file") or "")
    artifact_id = str(
        item.get("id") or item.get("artifact_id") or _stable_id_from_path(source_path)
    )
    artifact_type = str(item.get("artifact_type") or item.get("type") or "unknown")
    content_hash = (
        item.get("content_hash") or item.get("sha256") or _file_hash(repo_root, source_path)
    )
    ...
    return ArtifactNode(
        ...
        activation_signals=_as_tuple(
            item.get("activation_signals") or item.get("retrieval_keys") or item.get("tags")
        ),
        dependencies=_as_tuple(
            item.get("dependencies") or item.get("depends_on") or item.get("requires")
        ),
...
def edges_from_node(node: ArtifactNode) -> list[ArtifactEdge]:
    ...
    for signal in node.activation_signals:
        signal_id = "activation_signal." + re.sub(r"[^A-Za-z0-9_.-]+", "_", signal.lower()).strip(
            "_"
        )
        if signal_id != node.id:
            edges.append(
                ArtifactEdge(
                    node.id, signal_id, "activates_on", "activation_signals", confidence=0.85
                )
            )
    return edges
...
def verify_graph_seed(path: Path) -> dict[str, Any]:
    ...
    dangling_edges = [
        ...
        and not str(edge.get("source_id", "")).startswith("activation_signal.")
    ]
